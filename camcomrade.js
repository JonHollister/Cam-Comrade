...
/*The Interaction class handles the final step of the (detect phase)->(define targets)->(scan for targets)->(act upon flagged targets) bot loop. Provided with a single Botfather library Point object,the instance is constructed such that upon calling the .exec() method a single Tap is fired off to the ADB (Android Debug Bridge) system after a delay of between 100 and 275 milliseconds. Should the object however be constructed with a two Points, the .exec() method instead produces a Swipe from the first to the second Point with a duration between 300 and 500 milliseconds, following a pause similar to that which precedes the Tap version of the .exec() method. The purpose of the pause is to 'humanize' the interactions by randomizing the pauses between Taps / Swipes within reasonable bounds, as a lack of minute pauses between touch events - or a series if pauses with the exact same value each time - would be potentially characteristic of a bot.*/
class Interaction {
    
    constructor(tapPoint, swipePoint = null) {
        this.p1 = tapPoint;
        this.p2 = swipePoint;
...
class Asset {
    ...
    constructor(obj_in, color_in = "red", pix_in = 2) {
        this.needle = new Image(path(obj_in.file));
        this.mask = new Image(path(obj_in.file,,true));
...
/*The path() function takes one, two, or three args, corresponding to a file name, the folder within which it should be located, and a Boolean that determines whether "_mask" is appended to the intended name of the file. Using a template literal marked with backticks, variable names are substituted into a string that is then returned which contains a slash and the file type, namely that of a PNG. If only a single argument is passed in, the folder defaults to assets/, the location where all the target images are located, and no mask signifier is appended.*/
function path(fileName, inFolder = "assets", isMask) {
    let fullName = isMask ? fileName + "_mask" : fileName;
    return `${inFolder}/${fullName}.png`;
}
